...
그래서 페이징 테이블은 S-RAM으로 만들게 된다 이 때 이것을 TLB라고 부른다.<br>
...
## 가상메모리
메모리는 하드디스크 만큼 크지 않다. 그렇다면 메모리보다 큰 프로세스는 실행 할 수 없을까?<br>
아니다. 가상 메모리를 사용한다면 가능하다. 프로세스 이미지를 모두 메모리에 올리지 않고 필요한 부분만 메모리에 올려서 실행을 하고 필요할 때에 다시 메모리에 올려서 실행 하면 가능하다 <br>
#### Demand Paging
현재 가상메모리와 비슷한 개념으로 사용되고 있다. 프로세스 이미지는 backing store에 저장하고 요구되는 페이지만 메모리에 올리는 개념이다. <br>
페이징 테이블에 valid 비트 추가를 한다. <br>
페이지 결함(page fault)가 되면 backing store에서 해당 페이지를 가져온다. <br>
pure demand paging vs prepaging (미리 가져오냐 아니면 아무것도 안가져와서 실행하나)<br>
swapping vs demand paging (프로세스 단위 vs 페이지 단위 swap)<br>
#### 유효접근시간
page fault가 일어날 확률과 page fault가 일어날 시 디스크에서의 seek time을 곱해서 구한다. <br>
실제로는 page fault가 잘 일어나지 않는다. locality of reference때문에. 메모리 접근은 시간적, 공간적 지역석을 가진다.<br>
HDD 는 접근 시간이 너무길다. SSD 또는 DRAM을 사용하면 seek time 을 줄일수 있다. <br>
### 페이지 교체
페이지 폴트가 일어나면 backing store에서 필요한 페이지를 가져와야 한다. 만약에 메모리가 비어있다면 그냥 가져오면 되지만, 꽉 차 있을 경우에는 메모리에 있는 특정 페이지를 몰아내야한다. <br>
이때 몰아 내지는 페이지를 victim page라고 한다. <br>
그럼 어느 페이지를 몰아내야 할까?<br>
만약에 modified된 페이지를 몰아낸다면 하드디스크에서 data를 수정해야한다. 그렇기 때문에 되도록 modified되지 않은 페이지를 몰아내는 것이 더 이득 일 것이다. <br>
페이지 선택 algorithms으로는 random, first-in first-out Optimal,LRU등이 있다. <br>
FIFO는 말그대로 먼저 메모리에 들어온 페이지를 내보내는 것이다. 가장 간단하지만 성능이 썩 뛰어나진 않다. <br>
Optimal 같은 경우는 가장 이상적이고 효율이 가장 좋지만 현실적이지 못하다. 미래에 가장 사용 안될 페이지를 알야하지만 이것은 미래를 예측해야 하기 때문에 현실적이지 못하다 <br>
LRU같은 경우는 최근에 가장 사용되지 않은 페이지를 몰아내는 정책이다. 최근에 사용되지 않으면 앞으로도 사용이 될 확률이 적다는 생각으로 만들어진 알고리즘이다. <br>
Global<br>
메모리 상의 모든 프로세스 페이지에 대해 교체이고
Local Replacement <br>
메모리 상의 자기 프로세스 페이지에 대해 교체 이다. <br>
성능은 global replacement가 좀더 효율적일 수 있다. <br>

### 프레임 할당
처음 프로세스가 실행 될때 메모리에 있는 프레임에 페이지를 할당해야하는데 어떻게 할당해야 효율적일까?<br>
일반적으로 프로세스 개수가 증가하면 비례해서 cpu 이용률이 증가하는데 일정 범위를 넘어서면 cpu 이용률이 급 감소하게 된다. <br>
빈번한 page in/out이 일어나기 때문이다. 이것을 Thrasing이라고 한다. <br>
쓰레싱을 극복하기 위해서는 global 보다는 local replacement를 사용해야한다. <br>
그리고 처음에 프로세스당 충분한 수의 메모리를 할당해야한다. <br>

할당 방법에는 정적할당,동적 할당이 있다. <br>
동적할당이 조금더 효율적인 할당이다. <br>
#### 동적 프레임 할당
Working set model<br>
워킹셋 모델은 지역성(locality) 을 기반으로 가장 많이 쓰이는 페이지 집합을 메모리 공간에 계속 상주시켜 스레싱을 줄이는 방법을 말한다.<br>
어느정도의 크기의 집합을 만들지는 working set window가 설정해준다. <br>
Page-Fault Frequency(PFF)<br>
page fault 발생 비율의 상한/하한선을 정하고 특정 프로세스의 page fault가 상한성을 넘으면 더 많은 프레임을 할당해주고 하한선 이하로 page fault가 일어나면 프레임을 회수한다. <br>
### page size
페이지의 크기는 점차 커진다. (프로세스가 커지니까 당연)<br>
페이지가 커지면 내부 단편화는 더 심해지고, page-in,out 시간은 줄어든다. <br>
memory resolution은 감소한다.(필요없는 부분도 적재될 확률이 큼)<br>
page fault 발생 확률은 적어진다.<br>
페이지 테이블은 원래 tlb캐시로 만들어 졌지만 기술이 발전하면서 chip형태로 레지스터에 내장 되게 되었다. <br>
